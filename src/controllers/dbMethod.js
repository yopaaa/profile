...
class Database {
  constructor(collections = "", schema = { _id: String }, index = null) {
    ...
    const database = mongoose.models[collections] || mongoose.model(collections, dbSchema)
    // database.createCollection()
    // database.listIndexes((err, result) => {
    //   if (err) return console.log(err.message, 500)
    //   console.log(`${collections} list indexes ${result.length}`, 200)
    // })
    // database.countDocuments((err, result) => {
    //   if (err) return console.log(err.message, 500)
    //   console.log(`database ${collections} data count is = ${result}`, 200)
    // })
